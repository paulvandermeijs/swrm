...
`terminal::Terminal` wraps a `Backend` that owns `alacritty_terminal::term::Term<SwrmListener>` and an `EventLoop` powering `alacritty_terminal::tty::new`. Events (cursor moves, mode changes, bell, exit) flow through `SwrmListener` as a `futures::channel::mpsc::unbounded` channel. Snapshots are fetched via `Terminal::snapshot()`, which locks the term and flattens visible cells into `{ ch, fg, bg, flags }`. `terminal::render::render_snapshot` walks each row and batches contiguous cells of equal `(fg, bg, flags)` into one styled `div` per run; the renderer applies bold / italic / underline from `CellFlags`. Keymap encoding in `terminal::input::encode` is `TermMode`-aware (app-cursor / app-keypad arrows, F1-F12, alt-as-meta).
