...
`terminal::Terminal` wraps a `Backend` that owns `alacritty_terminal::term::Term<SwrmListener>` and an `EventLoop` powering `alacritty_terminal::tty::new`. Events (cursor moves, mode changes, bell, exit) flow through `SwrmListener` as a `futures::channel::mpsc::unbounded` channel. Snapshots are fetched via `Terminal::snapshot()`, which locks the term and flattens visible cells into `{ ch, fg, bg, flags }`. `terminal::render::render_snapshot` is a custom `gpui::canvas` paint (not a div tree). The paint callback walks each row, paints background quads via `window.paint_quad(fill(...))` for contiguous same-bg runs, then builds a `Vec<TextRun>` per row and calls `window.text_system().shape_line(text, font_size, runs, Some(cell_width))` followed by `shaped.paint(...)`. The `Some(cell_width)` argument is the key — it's gpui's `force_width`, which makes every glyph advance by exactly `cell_width` regardless of the font's natural advance, so the character grid stays pixel-aligned across rows even when fallback fonts (powerline arrows, etc.) have different metrics. Earlier div-based renderers either clipped trailing glyphs (when run width was constrained tightly) or showed visible gaps between runs (when run width was ceiled). All four JetBrains Mono variants (Regular, Bold, Italic, BoldItalic) are bundled and registered in `main.rs`, and `CellFlags::{BOLD, ITALIC}` toggle the run's `Font::weight`/`Font::style` so bold/italic stay monospace. `CellFlags::UNDERLINE` populates the run's `underline` style. Keymap encoding in `terminal::input::encode` is `TermMode`-aware (app-cursor / app-keypad arrows, F1-F12, alt-as-meta).
